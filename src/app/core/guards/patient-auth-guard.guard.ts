...
export const patientAuthGuardGuard: CanActivateFn = (route, state) => {
  ...
  if (token) {
    try {
      ...
      if (decodedToken.role.toLowerCase() !== 'patient') {
        console.log('User role is not patient, access denied');
